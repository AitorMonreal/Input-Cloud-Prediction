...
for i in range(len(vals)):
    ...
    os.chdir('C:/Users/monre/OneDrive - Imperial College London/ME4/FYP/code/SSM/Results/Point Cloud/' + str(faces) + '_faces_more')
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(transformed_model)
    o3d.io.write_point_cloud('femur_prediction' + str(PCs) + '.ply', pcd)

    np.save('params' + str(PCs) + '.npy', final_params)
    np.save('iterations' + str(PCs) + '.npy', iterations)
    np.save('error' + str(PCs) + '.npy', final_error)
    np.save('time' + str(PCs) + '.npy', final_time)
...
np.save('total_iter.npy', np.array(total_iter))
np.save('total_time.npy', np.array(total_time))
